...
import { fetchDiscountedProductsHomeData } from "../../redux/slices/products.slice";
import Product from "../../components/Product/Product";
import ProductSkeleton from "../../components/Product/ProductSkeleton";

const DiscountedPagination = () => {
  const dispatch = useDispatch();
  const { discountedProducts } = useSelector((state) => state.homeProducts);
  ...
  useEffect(() => {
    dispatch(fetchDiscountedProductsHomeData({ page: currentPage, pageSize }));
  }, [dispatch, pageSize]);
  ...
  return (
    <div className="home-products__list-second">
      <div className="home-products__list-top">
        <h1>Discounted products</h1>
        <div className="products-pagination__buttons">
          <button className="products-pagination__button-prev" onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1 || !discountedProducts || discountedProducts.status === "loading"}>
          ...
          <button className="products-pagination__button-next" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage * pageSize >= totalProducts}>
            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="14" viewBox="0 0 40 14" fill="none">
              <path
                opacity="0.4"
...
export default DiscountedPagination;
